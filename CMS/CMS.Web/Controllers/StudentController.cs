...
namespace CMS.Web.Controllers
{
    ...
    public class StudentController : BaseController
    {
        ...
        public StudentController(IClassService classService, ILogger logger, IRepository repository,
            ...
            ISchoolService schoolService, IBranchService branchService, IClientService clientService, IAspNetRoles aspNetRolesService,
            IBranchAdminService branchAdminService, IClientAdminService clientAdminService, ITeacherService teacherService,
        ...
        public ActionResult Index(int? id)
        {
            ...
            var projection1 = roles == "Client" ? _clientAdminService.GetClientAdminById(roleUserId) : null;
            ...
            ViewBag.ClassId = id;
            var students = (roles == "Admin" && id == null)
                            ? _studentService.GetAllStudents().ToList()
                            : (roles == "Admin" && id != null)
                            ? _studentService.GetStudentsByClassId((int)id).ToList()
                            : (roles == "BranchAdmin" && id == null)
                            ? _studentService.GetStudentsByBranchId(projection.BranchId).ToList()
                            : (roles == "BranchAdmin" && id != null)
                            ? _studentService.GetStudentsByBranchAndClassId((int)id, projection.BranchId).ToList()
                            : null;
            ...
            {
                ViewBag.userId = projection1.ClientId;
            }
            else
            {
